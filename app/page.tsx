      ...
      <main className={currentStep === 'game' ? 'flex-1 w-full flex flex-col items-center justify-center' : 'max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8 flex-1 flex flex-col justify-center w-full'}>
        ...
        {currentStep === 'instructions' && (
          <div className="w-full max-w-2xl mx-auto bg-white rounded-[2rem] p-5 sm:p-8 border border-orange-100 shadow-sm" id="step-instructions">
            ...
            {/* Standardized 5x5 Tumbling E Clinical Specifications Reference */}
            <div className="mb-6 p-3.5 sm:p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between gap-3 sm:gap-4 shadow-2xs">
              <div className="flex items-center gap-3">
                <div
                  suppressHydrationWarning={true}
                  className="w-11 h-11 sm:w-12 sm:h-12 bg-white rounded-xl border border-slate-200 flex items-center justify-center p-1 shrink-0"
                >
                  <StandardTumblingE
                    orientation="right"
                    sizeMm={4.37}
                    pxPerMm={currentPxPerMm}
                    id="spec-preview-optotype"
                  />
                </div>
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-800">
                    Standardized 5×5 Tumbling E • 1.00 m Distance
                  </h4>
                  <p className="text-[11px] text-slate-600 font-medium leading-tight mt-0.5">
                    Letter: 4.37 mm × 4.37 mm • Stroke: 0.874 mm • 6/18 (≈0.48 logMAR) • 15 arcmin
                  </p>
                </div>
              </div>
              <span className="hidden sm:inline-flex px-2 py-1 rounded bg-slate-200 text-slate-700 text-[10px] font-extrabold uppercase tracking-wide shrink-0">
                5×5 Optotype
              </span>
            </div>
